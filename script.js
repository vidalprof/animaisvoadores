 ...
 * - RECONSTRUÍDA a lógica de áudio para funcionar como um 'toggle' (toca/para).
 * - REFORÇADA a lógica para interromper o áudio automaticamente ao navegar para outra tela.
 */
...
let foundWords = [];
const gridSize = 12;
let somAtual = null; // Variável para controlar o som que está tocando
...
document.getElementById('btn-comecar').addEventListener('click', () => { /* ...código idêntico... */ });
function carregarGridAnimais() { /* ...código idêntico... */ }
function mostrarFicha(idAnimal) { /* ...código idêntico... */ }

// --- LÓGICA DO ÁUDIO (RECONSTRUÍDA) ---
function tocarSom(idAnimal) {
    // Se o usuário clicar no botão do som que já está tocando, ele para.
    ...
    // Se outro som estiver tocando, ele para o antigo antes de tocar o novo.
    player.pause();
    player.currentTime = 0;
    ...
    if (promise !== undefined) {
        promise.then(_ => {
            somAtual = idAnimal; // Marca o som que está tocando
        }).catch(error => {
...
// ... (Todas as funções do caça-palavras)

// --- LÓGICA DO QUIZ (sem alterações) ---
// ... (Todas as funções do quiz)

// --- LÓGICA DO RELATÓRIO FINAL (sem alterações) ---
// ... (Todas as funções do relatório)
